...
import { useBlockProps, InspectorControls, MediaUpload, MediaUploadCheck, URLInputButton, RichText } from '@wordpress/block-editor';
...
const TEXT_FORMATS = ['core/bold', 'core/italic', 'core/underline'];
          ...
          <>
            ...
            {isLinkedText ? (
              <p className="item-text mbr-fonts-style display-7">
                <a className="item-text-link" href={safeDetail.url}>
                  <RichText.Content tagName="span" value={safeDetail.text} />
                </a>
              </p>
            ) : (
              <p className="item-text mbr-fonts-style display-7">
...
function Edit({ attributes, setAttributes }) {
  const {
    ...
    label, heading, text, details, imageUrl
  } = attributes;
    ...
    <div {...useBlockProps()}>
      <InspectorControls>
        ...
        <PanelBody title={__('Content', 'restatify-base')} initialOpen={false}>
          ...
          />
          <p className="restatify-detail-link-label">
            {__('Text can be edited directly in the block preview (line breaks + basic formatting).', 'restatify-base')}
          </p>
        </PanelBody>

        <PanelBody title={__('Details', 'restatify-base')} initialOpen={false}>
          <div className="restatify-detail-actions">
            <Button variant="primary" onClick={() => setAttributes({ details: addDetail(normalizedDetails) })}>
              {__('Add detail', 'restatify-base')}
            </Button>
          </div>

          {normalizedDetails.map((detail, index) => (
            <div className="restatify-detail-editor" key={`detail-${index}`}>
              <TextControl
                label={__('Label', 'restatify-base')}
                value={detail.title}
                onChange={(v) => setAttributes({ details: updateDetail(normalizedDetails, index, { title: v }) })}
              />
              <SelectControl
                label={__('Display mode', 'restatify-base')}
                value={detail.mode}
                options={DETAIL_MODE_OPTIONS}
                onChange={(v) => setAttributes({ details: updateDetail(normalizedDetails, index, { mode: v }) })}
              />

              {detail.mode !== 'button' && (
                <>
                  <p className="restatify-detail-link-label">{__('Text', 'restatify-base')}</p>
                  <div className="restatify-richtext-input">
                    <RichText
                      tagName="p"
                      value={detail.text}
                      allowedFormats={TEXT_FORMATS}
                      multiline={false}
                      onChange={(v) => setAttributes({ details: updateDetail(normalizedDetails, index, { text: v }) })}
                      placeholder={__('Add detail text…', 'restatify-base')}
                    />
                  </div>
                </>
              )}
              ...
              {detail.mode !== 'text' && (
                <>
                  <p className="restatify-detail-link-label">{__('Link', 'restatify-base')}</p>
                  <URLInputButton
                    url={detail.url}
                    onChange={(url) => setAttributes({ details: updateDetail(normalizedDetails, index, { url: url || '' }) })}
                  />
                </>
              )}

              <div className="restatify-detail-order-controls">
                <Button
                  variant="secondary"
                  disabled={index === 0}
                  onClick={() => setAttributes({ details: moveDetail(normalizedDetails, index, index - 1) })}
                >
                  {__('Move up', 'restatify-base')}
                </Button>
                <Button
                  variant="secondary"
                  disabled={index === normalizedDetails.length - 1}
                  onClick={() => setAttributes({ details: moveDetail(normalizedDetails, index, index + 1) })}
                >
                  {__('Move down', 'restatify-base')}
                </Button>
                <Button
                  variant="secondary"
                  isDestructive
                  onClick={() => setAttributes({ details: removeDetail(normalizedDetails, index) })}
                >
                  {__('Remove', 'restatify-base')}
                </Button>
              </div>
            </div>
          ))}
        ...
        <PanelBody title={__('Image', 'restatify-base')} initialOpen={false}>
            ...
            <MediaUpload
              ...
              render={({ open }) => (
                <Button variant="secondary" onClick={open}>
                  {imageUrl ? __('Replace image', 'restatify-base') : __('Choose image', 'restatify-base')}
                </Button>
              )}
            />
          </MediaUploadCheck>
          {imageUrl && (
            <Button variant="link" onClick={() => setAttributes({ imageUrl: '' })}>
              {__('Remove image', 'restatify-base')}
            </Button>
          )}
        </PanelBody>
      </InspectorControls>
      ...
      <div className={className} style={style}>
        {backgroundImageEnabled !== false && overlayEnabled && (
          <div className="restatify-contact-bg-overlay" style={overlayStyle} aria-hidden="true"></div>
        )}
            ...
            <div className="col-12 col-lg-11">
              <div className="content-wrapper">
                <RichText
                  ...
                  multiline={false}
                  onChange={(v) => setAttributes({ label: v })}
                  placeholder={__('Add label…', 'restatify-base')}
                />
                <h2 className="mbr-section-title mbr-fonts-style display-2">
                  <strong>
                    <RichText
                      ...
                      multiline={false}
                      onChange={(v) => setAttributes({ heading: v })}
                      placeholder={__('Add heading…', 'restatify-base')}
                    />
                  </strong>
                </h2>
                ...
                <RichText
                  ...
                  placeholder={__('Add intro text…', 'restatify-base')}
                />
                ...
                <div className="row content-wrap">
                  <div className="col-12 col-lg-6 card">
                    <div className="items-wrapper">
                      {normalizedDetails.map((detail, index) => (
                        <ContactItem key={`contact-item-${index}`} detail={detail} />
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="col-12 col-lg-6 card">
                    <div className="image-wrapper">
                      {imageUrl ? (
                        ...
                        <div className="image-placeholder" aria-label={__('Choose image', 'restatify-base')}></div>
                      )}
                    </div>
                  </div>
                </div>
...
function Save({ attributes }) {
  ...
  const blockProps = useBlockProps.save({
    className: ['restatify-contact-block', backgroundImageEnabled !== false && parallax ? 'is-parallax' : '', fullscreen ? 'is-fullscreen' : ''].join(' '),
        ...
        <div className="row justify-content-center">
          <div className="col-12 col-lg-11">
            <div className="content-wrapper">
              ...
              {text && <RichText.Content tagName="p" className="mbr-text mbr-fonts-style display-4" value={text} />}
              ...
              <div className="row content-wrap">
                ...
                <div className="col-12 col-lg-6 card">
                  <div className="image-wrapper">{imageUrl && <img src={imageUrl} alt="" loading="lazy" />}</div>
                </div>
              </div>
            </div>
          </div>
